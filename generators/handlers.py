...
def _playsound(sound: str | None) -> list[str]:
    if not sound:
        return []
    # player-relative UI click / feedback
    return [f"playsound {sound} master @s ~ ~ ~ 1 1"]


def _run_functions(functions: list[str]) -> list[str]:
    return [f"function {fn}" for fn in functions]


def _emit_actions(w: Widget, prefix: str = "") -> list[str]:
    """Emit commands + functions + sound, optionally wrapped with an execute prefix."""
    lines: list[str] = []
    for c in w.commands:
